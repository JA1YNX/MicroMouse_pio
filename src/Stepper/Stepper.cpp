...
{
    pinMode(pin.p_M1, OUTPUT);//1/0
    pinMode(pin.p_M2, OUTPUT);//1/0
    pinMode(pin.p_M3, OUTPUT);//1/0
    pinMode(pin.p_CLOCK,OUTPUT);//PWM or 1/0 with timer(Ticker)
    pinMode(pin.p_ref, OUTPUT);//1/0 for sleep
    pinMode(pin.p_reset, OUTPUT);//1/0 for reset
    pinMode(pin.p_dir, OUTPUT);//1/0  CW/CCW
    pinMode(pin.p_SYNC, OUTPUT);//1/0 SYNC or Not SYNC
    pinMode(pin.p_Flag, INPUT);//Over Current Flag input

    digitalWrite(pin.p_reset, 0);//disable reset
    digitalWrite(pin.p_CLOCK, 0);
    digitalWrite(pin.p_ref, 0);
    digitalWrite(pin.p_SYNC, 1);//SYNC
    digitalWrite(pin.p_dir, 0);//CW

    ledcSetup(0, freq, resolution);
    ledcAttachPin(pin.p_CLOCK, 0);
    ledcWrite(0, 0);

    set_DriveMode(driveMode::dual_ModeF);
}
